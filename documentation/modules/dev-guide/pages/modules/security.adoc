...
Normally, form client is the default client used for authentification.
When an authentification provider is defined as exclusive, form client is not loaded at all.
...
Beware that some providers, such as Keycloak, uses `private_key_jwt` authentication method by default.
If you don't want that, you may use `client_secret_basic` authentication method instead:
...
If you want to use the `private_key_jwt` authentication method, you _must_ provide the `PrivateKeyJWTClientAuthnMethodConfig` component:
...
Note that to set the `PrivateKeyJWTClientAuthnMethodConfig` component, a custom converter is used instead of reflection.
