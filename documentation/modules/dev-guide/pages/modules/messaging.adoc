...
- **application.audit.processor.batch-delay**: An interval in millisecond. The pause taken after successfully
...
- **application.audit.processor.activity-windows**: An interval in millisecond. Defines the time window to consider
the system "Busy" after a user action. Default to `200`.
- **application.audit.processor.busy-backoff-interval**: An interval in millisecond. The amount of time the processor
...
- **application.audit.processor.batch-size**: AuditProcess batch size. Defines the number of audit log groups fetched
