...
* `x-step`: amount to increment or decrement on each step. Accept negative and decimal values. Default
value is determined by the scale attribute if it exists, otherwise it is set to `1`.
