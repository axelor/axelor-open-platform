...
* `x-stepper-show-description`: If true, display description below label. Works only for `selection` fields with `data-description` attribute. Default to `false`.
* `x-stepper-type`: If value is "icon", display icon instead of numeric indicator. Works only for `selection` fields with `icon` attribute. Default to `numeric`.
