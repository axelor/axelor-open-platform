...
The date widget is used for `Date` data fields.
The field provides the ability to select a date with a calendar popover in edit mode.
...
Time widget supports following attribute:
...
Date time widget supports following attribute:

* `x-seconds`: Show seconds. Default to false.

[source,xml]
----
<field name="updatedOn" widget="DateTime" />
----

== RelativeTime
 
The relative time widget is used for `Date` and `DateTime` data fields.

The field renders relative time from now.

[source,xml]
----
<field name="orderDate" widget="RelativeTime" />
...
Duration widget supports following attributes:
