...
The `Uuid` widget is used for `uuid` data fields. The input provides a mask for user guidance, helping to format
the value correctly.

[source,xml]
----
<field name="productUid" widget="Uuid"/>
----

image::widgets/uuid.png[]
...
* `x-auto-size`: if `true`, the editor auto-sizes to fit content, growing up to the specified `height` (defaults to 360px).
* `x-lite`: if `true`, use a minimal editor with no line numbers, folding, hover, suggestions, or context menu.
