...
<<<<<<< HEAD
* `x-color-field`: specify the tag color field that should either use selection `color.name.selection`, or be a hexadecimal value.
=======
...
>>>>>>> 5e8df93c9 (Add hexadecimal support on x-color-field)
