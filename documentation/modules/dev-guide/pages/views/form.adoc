...
A `panel-dashlet` can be used to embed other views like chart, cards, iframe…

A `panel-dashlet` can have the following attributes:
...
| `canEdit` | whatever we can edit dashlet records even if the dashlet is in readonly
| `canDelete` | whatever we can delete dashlet records
| `canNew` | whatever we can create dashlet records
...
| `showTitle` | whether to show the panel title
| `if-module` | use the widget if the given module is installed
| `x-show-bars` | whether to show the related grid toolbar and menubar in header
|===
...
<!-- Display a grid -->
<panel-dashlet action="sale.category.link.products" />
-----

==== Search request

The domain context of the search request is the main top record.

[source,xml]
-----
<panel-dashlet action="sale.category.link.products" />

<action-view name="sale.category.link.products" model="com.axelor.sale.db.Product" title="Related products">
    <view type="grid"/>
    <view type="form"/>
    <domain>self.category.id = :_categoryId</domain>
    <!-- id is the id of the main top record (ie, the category) -->
    <context name="_categoryId" expr="eval: id"/>
</action-view>
-----

==== Toolbar and menubar actions

The grid toolbar and menubar actions context is the dashlet context containing :

* the action view context
* the main top record in `_parent`
* if any records are selected, the selected line ids are available in `_ids`

Since these actions run in the grid context, the action response behaves as follows:

* `response.setReload(true)` reloads the dashlet only.
* `response.setSignal("refresh-tab", null)` refreshes the active tab.
* `response.setValue(...)` has no effect, as there is no record to update in the
  grid context.
* `response.setAttr(...)` currently has no effect.
