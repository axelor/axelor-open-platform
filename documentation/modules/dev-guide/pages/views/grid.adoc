...
| `widget` | name of the widget to be used in the grid: `expandable`
...
| `onSave` | action to be called on saving form in editable grid
| `onDelete` | action to be called when selected records are deleting
|===
...
NOTE: See xref:web-client/widgets.adoc#expandable[Expandable] for usage and available widget attributes. It's the only widget supported on top-level grid.
