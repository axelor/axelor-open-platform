...
* `maxSeries` - maximum number of series (dimensions) to display. When the dataset produces more
  series than this limit, only the top N series by value are shown and the remaining ones are
  aggregated into an "Others" bucket. Defaults to `50`. Set to `0` to disable the limit.
  This is useful for charts with large datasets to prevent excessive memory consumption.
...
series by keeping the top N by value and aggregating the rest into a single "Others" series.
...
The default value is `50`. To disable the limit entirely, set it to `0`:
