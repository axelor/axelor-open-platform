...
requires actions to do something useful other then just CRUD operations.
...
Actions are re-usable. So each action should be given an unique name.

== Events

Actions can be performed on various events. These includes:
...
* `is(Object, Class)` - check whether the given object is instance of the the class
...
Some built-in variables are available to be used with expressions. These includes:
...
- `sync` - to sync record, can be used at the beginning or end only
