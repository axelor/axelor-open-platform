...
| `--tld-scan-jars=<tldScanJars>[,<tldScanJars>...]`
| Comma-separated jar name patterns (e.g. `jstl-*.jar,taglibs-*.jar`) to include when scanning for JSP taglib
descriptors (`META-INF/**/*.tld`).

| `--pluggability-scan-jars=<pluggabilityScanJars>[,<pluggabilityScanJars>...]`
| Comma-separated jar name patterns (e.g. `axelor-*.jar`) to include when scanning for Servlet 3.0 pluggability
(`META-INF/web-fragment.xm`l and `@WebServlet`/`@WebFilter`/`@WebListener` annotations).
|===
...
* **Hashing algorithm upgrade** — migrating from the legacy `$AES$` format (PBKDF2WithHmacSHA1) to the new
`$AESv1$` format (PBKDF2WithHmacSHA256). No extra configuration is needed: the password and AES algorithm remain
the same, only the hashing algorithm changes.

[source,sh]
----
  $ axelor database encrypt [OPTIONS]
----
...
| Specify the number of hash iterations used.
|===
