...
- `tenantAware()`: Enables data isolation per tenant. When multi-tenancy is active, the cache automatically segregates entries based on the current tenant context. It is critical that you enable this option if your cached data is database-related in any way.
...
private static final AxelorCache<String, Action> ACTIONS =
    CacheBuilder.newBuilder("actions").maximumSize(1000).weakValues().tenantAware().build(XMLViews::findAction);
----

Here, the cache is configured with a maximum size of 1000 entries and uses weak references for values.
Also, the `tenantAware()` option is enabled to ensure data isolation per tenant.
The `XMLViews::findAction` loader dynamically loads actions not present in the cache.
