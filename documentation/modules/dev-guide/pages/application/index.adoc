...
pluginManagement {
  ...
  plugins {
    id 'com.axelor.app' version '8.0.+' <1>
  }
}
...
----
<1> The axelor app gradle plugin version
<2> Use maven central
<3> The Node.js repository
...
The application also comes with an enterprise edition that brings new features such as studio, global search support,
advance import/export, etc... Documentation for EE-specific features is marked with: `(🏷️ EE only)`

To use the enterprise edition :

1. Contact the sales team to obtain a commercial license
...
[source,properties]
----
axelor_ee_registry_url=https://repository.axelor.com/nexus/repository/maven-enterprise/
axelor_ee_registry_user=<your-username>
axelor_ee_registry_password=<your-password>
----

3. Add EE maven repository to the project :
+
.settings.gradle
[source,gradle]
----
pluginManagement {
  repositories {
    ...
    mavenCentral()
    maven {
      url = uri(providers.gradleProperty("axelor_ee_registry_url").get())
      credentials {
        username providers.gradleProperty("axelor_ee_registry_user").get()
        password providers.gradleProperty("axelor_ee_registry_password").get()
      }
    }
  }
}
...
4. Enable enterprise edition in `gradle.properties` file:
+
.gradle.properties
[source,properties]
----
axelor.platform.ee = true
----

5. Build the application : `./gradlew -xtest clean build`
...
=== Check duplicate classes plugin

Duplicate classes in classpath can cause unexpected behavior at runtime. Generally, they come from transitive
dependencies, and most of the time we want to exclude them. The plugin checks whether different artifacts in a
configuration provide the same classes.

The plugin is automatically applied by the `AxelorPlugin`. If the module isn't an Axelor module, plugin can be
manually applied : `apply plugin: com.axelor.gradle.support.DuplicatedClassSupport`

Plugin can be configured with the following properties :

[source,gradle]
----
checkDuplicateClasses {
  ignoreFailures = false
  excludes = ["jakarta-action-api.*"]
  classpathToScan = configurations.runtimeClasspath
}
----
...
| `ignoreFailures` | Fails the build if any duplicate classes are found or not | false
| `excludes` | List of excluded jar name (regex) | []
| `classpathToScan` | The classpath to scan |  runtimeClasspath
|===

The check is performed by the task `checkForDuplicateClasses`. In output, it will print all libraries in conflicts.
