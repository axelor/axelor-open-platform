...
The application supports different cache providers that can be configured globally or for specific components (Hibernate, Shiro).

In the default cache configuration, the application uses the following:

[cols="1,3"]
|===
| Component           | Cache Provider

| Hibernate           | https://github.com/ben-manes/caffeine[Caffeine]
| Shiro               | https://shiro.apache.org/caching.html#memoryconstrainedcachemanager[MemoryConstrainedCacheManager]
| Internal components | https://github.com/ben-manes/caffeine[Caffeine]
|===
...
* `default` - Use default caching implementation that can vary by components
* `caffeine` - Use Caffeine caching
...
When using Caffeine, there is no configuration to provide.

When using Redisson, you can to provide a configuration file (defaults to `redisson.yaml`) via the `.config`-suffixed property that specifies the Redis connection details. Make sure your configuration file exists either in classpath or on file system.

Example minimal Redisson configuration file:
...
See https://redisson.org/docs/configuration/#using-yaml[Redisson configuration documentation] for details.
