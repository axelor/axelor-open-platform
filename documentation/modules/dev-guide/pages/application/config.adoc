...
The cache configuration currently supports three providers:

* `default` - Use default caching implementation that can vary by components
* `caffeine` - Use Caffeine caching
* `redisson` - Use Redis/Valkey through Redisson client
...
When using Redisson, you need to provide a configuration file (e.g. `redisson.yaml`) via the `.config`-suffixed property that specifies the Redis connection details.
