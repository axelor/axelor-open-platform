...
To use encrypted secrets, `props.encryptor.password` properties
...
* `props.encryptor.algorithm` :Algorithm to be used by the encryptor
* `props.encryptor.key-obtention-iterations` : Number of hashing iterations to obtain the signing key
* `props.encryptor.provider-name` : Name of the security provider to be asked for the encryption algorithm
* `props.encryptor.provider-class-name` : Security provider to be used for obtaining the encryption algorithm
* `props.encryptor.salt-generator-classname` : Salt generator to be used by the encryptor
* `props.encryptor.iv-generator-classname` : IV generator to be used by the encryptor
* `props.encryptor.string-output-type` : Sets the form in which String output will be encoded

The default algorithm is `PBEWITHHMACSHA512ANDAES_256`. Most of the time,
default encryption settings will be enough.
...
[source,bash]
----
$ ./gradlew :axelor-core:encryptText --text="A secret to encode" --password="MySecureKey"

-------Configs-------
props.encryptor.password = MySecureKey

WARNING : Do not add property `props.encryptor.password` with the password in your configuration file.
