...
:url-tomcat-jndi: https://tomcat.apache.org/tomcat-10.1-doc/jndi-datasource-examples-howto.html
...
----
<1> automatic schema generation option
...
The `db.default.ddl` configuration property is used to customize the Hibernate database schema generation process, and
it can take the following values:

- _none_ : Does not perform any schema generation or validation
- _create_ – Creates the database schema from scratch. This will drop any existing tables and recreate them based on
the mappings defined in the Hibernate configuration.
- _create-drop_ – Creates the database schema and drops it when the SessionFactory is closed.
- _validate_ – Validates the database schema against the mappings defined in the Hibernate configuration.
This will check that the database schema is in sync with the mappings, but it will not make any changes to the schema.
- _update_ – Updates the database schema to reflect any changes made to the mappings defined in the Hibernate
configuration. This will add new tables or columns, but it will not drop any existing tables or columns.

Choose the right option by environment :

- Production: `none` or `validate` for stability.
- Development: `update` is the go-to for flexibility.
- Testing: `create` or `create-drop` gives you fresh data every test.

=== Using a JNDI DataSource

By default, {page-component-title} applications manage their own database connection pool (HikariCP), using the above
database settings.

Alternatively, the application can use a `DataSource` managed by the servlet container and looked up by its JNDI name.

* Application configuration

First, remove the `db.default.url`, `db.default.user` and `db.default.password` settings from
`axelor-config.properties`, and set the JNDI name of the `DataSource`:

[source,properties]
----
db.default.datasource = java:comp/env/ds/AxelorDS
----

Alternatively, the JNDI name can be set with the `non-jta-data-source` element  of `META-INF/persistence.xml`:
...
`db.default.datasource` takes precedence over `non-jta-data-source`.

IMPORTANT: When a `DataSource` is used, `db.default.ddl` is ignored. Use `hibernate.hbm2ddl.auto` instead, for example
`hibernate.hbm2ddl.auto = update` in `axelor-config.properties`. The `hibernate.hikari.*` settings don't apply either:
the connection pool is configured in the container.

* Tomcat configuration

Copy the database JDBC jar to `$CATALINA_HOME/lib`.

Then declare the `DataSource` in the context of the application, in
`$CATALINA_BASE/conf/Catalina/localhost/axelor-demo.xml` (the file name must match the context path of the application,
here `/axelor-demo`):
...
To share a `DataSource` between several applications, declare the `<Resource>` in the `<GlobalNamingResources>`
element of `conf/server.xml`, and link it in the application context with a `ResourceLink` element:

[source,xml]
----
<Context>
  <ResourceLink global="ds/AxelorDS" name="ds/AxelorDS" type="javax.sql.DataSource"/>
</Context>
----

See {url-tomcat-jndi}[Tomcat documentation] for more details.
...
| `application.entity.sequence.default_allocation_size` | global default entity allocation size |
| `application.entity.sequence.<SEQUENCE_NAME>.allocation_size` | per-entity sequence allocation size |
...
| `application.script.timeout` | Groovy/JavaScript scripts execution timeout (in milliseconds) | 300000
...
| `encryption.password` | encryption password : use unique, long, and complex string |
| `encryption.algorithm` | encryption algorithm (CBC or GCM) | GCM
...
| `data.upload.temp-dir` | storage path for temporary files | \{java.io.tmpdir}/.axelor
...
| `data.store.provider` | FQN of a `Store` implementation |
...
| `quartz.job-store.class` | JobStore class (e.g., `org.quartz.impl.jdbcjobstore.JobStoreTX` for JDBC persistence) | `org.quartz.simpl.RAMJobStore`
| `quartz.job-store.cluster-checkin-interval` | cluster check-in interval in milliseconds | `7500`
| `quartz.data-source.maximum-pool-size` | maximum number of connections for JDBC JobStore's data source | `quartz.thread-count`
| `quartz.data-source.connection-test-query` | SQL validation query for JDBC JobStore's data source |
| `quartz.data-source.idle-timeout` | idle connection discard time in milliseconds for JDBC JobStore's data source | `0` (disabled)
...
Property `jakarta.persistence.sharedCache.mode` can be used to enable or disable second level caching.
...
By default, the `Caffeine` implementation is used with pre-configured settings defined in `application.conf`. If you need to customize or override these settings, you can add and edit `application.conf` in your application resources.

The default configuration defines the following cache regions and settings:

[cols="3,1,1,1"]
|===
| Region Name | Max Size | Expire After Write | Expire After Access

| `default` (fallback for all other regions)
| 1000
| 10m
| -
...
There are also popular caching libraries such as {url-ehcache}[Ehcache],
...
[source,gradle]
----
implementation "org.infinispan:infinispan-hibernate-cache-v66:${infinispan_version}"
...
Redis is built-in and doesn't need external dependencies.

[source,properties]
----
# Use RedissonRegionFactory/RedissonRegionNativeFactory or any subclass
...
# Alternatively, you can specify config file path
hibernate.cache.redisson.config = /path/to/redisson.yaml

# Optional cache settings
# Max size of cache (not supported by RedissonRegionNativeFactory)
hibernate.cache.redisson.entity.eviction.max_entries = 10000
# Time to live per cache entry in milliseconds
hibernate.cache.redisson.entity.expiration.time_to_live = 600000
# Max idle time per cache entry in milliseconds (not supported by RedissonRegionNativeFactory)
hibernate.cache.redisson.entity.expiration.max_idle_time = 300000
----

When a Redisson-based region factory is used (`RedissonRegionFactory`), default cache region settings equivalent to the Caffeine defaults above are applied.
With the native eviction version (`RedissonRegionNativeFactory`), only time-to-live settings are supported, so it is applied in place of the max idle time.
Any `hibernate.cache.redisson.*` settings defined in the application configuration take precedence over these defaults.

More details on https://redisson.org/docs/cache-api-implementations/#hibernate-cache[Redisson Hibernate Cache documentation].

[NOTE]
Instead of manually setting `hibernate.cache.region.factory_class`, you can set `application.cache.provider = redisson` or `application.cache.hibernate.provider = redisson` for better integration by reusing the same Redisson client across all the app components. See <<Global and Component-specific Cache Configurations>>.
...
Make sure to add required dependencies if you want to use another caching library. Refer to providers docs for
extensive configuration : all properties prefix by `hibernate.` will be used.

== Global and Component-specific Cache Configurations

The application supports different cache providers that can be applied globally or for specific components (Hibernate, Shiro).
In the default cache configuration, the application globally uses Caffeine cache.
The cache configuration allows to override those defaults, and is done through the following properties:

[source,properties]
----
# Global cache configuration applied to Hibernate, Shiro and internal components
# ~~~~~
# Global cache provider: default/caffeine/redisson/redisson-native
application.cache.provider = redisson
# Global cache configuration for specified provider (if required by provider): either on filesystem or resource in classpath
application.cache.config.path = redisson.yaml
...
#application.cache.hibernate.config.path = redisson.yaml
...
#application.cache.shiro.config.path = redisson.yaml
----

The cache configuration currently supports these providers:

* `default` - Use default caching implementation
* `caffeine` - Use Caffeine caching (same as default)
* `redisson` - Use Redis/Valkey through Redisson client using scripted eviction
* `redisson-native` - Use Redis/Valkey through Redisson client using native eviction

[NOTE]
Redisson native eviction allows to clean up expired map entries on Redis side instead of using a scheduled task.
It requires https://valkey.io/commands/hpexpire/[HPEXPIRE command support] (Redis 7.4+ and Valkey 9.0+) and is currently supported for app-level cache and Hibernate cache, but not for Shiro cache.

For global cache configuration, using one of those names will use the appropriate internal customized providers for each component for optimal usage (such as reusing the same Redisson instance).

In the case of component-specific configurations (Hibernate, Shiro), you also have the option to specify the provider as fully-qualified class name in case you have custom requirements. For Hibernate, it can be either a `org.hibernate.cache.spi.RegionFactory` or `javax.cache.spi.CachingProvider` for JCache usage. For Shiro, it should be a `javax.cache.spi.CachingProvider` for JCache usage only.

When using Caffeine, there is no cache provider configuration to provide.

When using Redisson, there are two ways to provide Redisson configuration:

1) Specify the path to a configuration file (defaults to `redisson.yaml`).
Use `application.cache.config.path` for application-global cache configuration file, `application.cache.hibernate.config.path` for Hibernate-specific cache configuration file, and `application.cache.shiro.config.path` for Shiro-specific cache configuration file.
Make sure your configuration file exists either in classpath or on file system.

Example Redisson configuration file `redisson.yaml`:
...
2) Specify the configuration values directly within `axelor-config.properties`.
Use `application.cache.config.` prefix for application-global cache configuration, `application.cache.hibernate.config.` prefix for Hibernate-specific cache configuration, and `application.cache.shiro.config.` prefix for Shiro-specific cache configuration.
Configuration sub-keys are used to programmatically set the Redisson configuration via Java reflection.
The syntax for values differs slightly from the YAML Redisson configuration file, such as using comma-separated strings for lists and plain fully-qualified class names for bean instances.

Example embedded Redisson configuration:

[source,properties]
----
application.cache.config.single-server-config.address = redis://127.0.0.1:6379
application.cache.config.single-server-config.idle-connection-timeout = 10000
application.cache.config.single-server-config.connect-timeout = 10000
application.cache.config.single-server-config.timeout = 3000
application.cache.config.single-server-config.retry-attempts = 3
application.cache.config.single-server-config.retry-interval = 1500
application.cache.config.threads = 16
application.cache.config.netty-threads = 32
application.cache.config.codec = org.redisson.codec.SerializationCodec
----

See https://redisson.org/docs/configuration/[Redisson configuration documentation] for details about Redisson configuration.

See xref:dev-guide:application/cache.adoc[AxelorCache] for cache usage in application code.

[WARNING]
====
When using a distributed cache (such as Redisson), cached entity data is stored externally in serialized form. After changing your entities (e.g. installing or upgrading modules, or adding, removing or altering fields) or after editing the database manually, previously cached data may become incompatible or stale. In such cases, you must manually clear the distributed cache (e.g. flush the Redis/Valkey instance) to avoid deserialization errors or inconsistent data.
====
...
For links, you may use `:username` in the URL to have the username as a parameter.

[#password-requirements]
== Password Requirements

Password policies enforce constraints on passwords when a user sets or changes their password.

=== Built-in Policies

Three policies are active by default.

Two of them are *mandatory* and cannot be disabled via configuration:

`length`::
Passwords must be at least 8 characters long. Leading and trailing spaces are ignored to prevent
space-padding bypasses. The minimum length is configurable:
+
[source,properties]
----
user.password.length.min = 8
----
+
NOTE: The password field in the database has a minimum column length of 4 characters. Configuring
a value below this threshold has no effect — the policy will silently enforce 4 as the minimum.
It is recommended to always keep this value at or above the default of 8.

`notSame`::
A new password must differ from the user's current password. This policy is always enforced and
has no configuration parameters.

The third default policy is active by default but can be disabled:

`notCode`::
The password must not contain the user's login code (case-insensitive substring check).
+
[source,properties]
----
# Disable the "must not contain login code" check
user.password.notCode.enabled = false
----

=== Opt-in Policies

Several additional policies are available but disabled by default. They are designed to cover
common security requirements without imposing them on all deployments. Each policy is identified
by a short ID and can be switched on with a single property:

[source,properties]
----
user.password.<policy-id>.enabled = true
----

The available opt-in policies are:

`digits`::
Requires the password to contain a minimum number of digit characters.
+
[source,properties]
----
user.password.digits.enabled = true
user.password.digits.min = 1
----

`lowerCase`::
Requires the password to contain a minimum number of lowercase letters.
+
[source,properties]
----
user.password.lowerCase.enabled = true
user.password.lowerCase.min = 1
----

`upperCase`::
Requires the password to contain a minimum number of uppercase letters.
+
[source,properties]
----
user.password.upperCase.enabled = true
user.password.upperCase.min = 1
----

`specialChars`::
Requires the password to contain a minimum number of special (non-alphanumeric) characters.
+
[source,properties]
----
user.password.specialChars.enabled = true
user.password.specialChars.min = 1
----

`pattern`::
Requires the password to match a custom regular expression.
+
[source,properties]
----
user.password.pattern.enabled = true
user.password.pattern.value = (?=.*[A-Z])(?=.*\d).+
----

`score`::
Requires the password to reach a minimum strength score (0 = very weak, 4 = very strong) as
computed by the https://github.com/nulab/zxcvbn4j[zxcvbn] library.
+
[source,properties]
----
user.password.score.enabled = true
user.password.score.min = 2
----

=== Custom Password Policy

You can provide your own password policy by implementing the `PasswordPolicy` interface :
...
@Singleton
public class MyCustomPasswordPolicy implements PasswordPolicy {

  private static final String ID = "my-custom";
  private static final String DESCRIPTION = "My custom password policy";
  private static final String ERROR_MESSAGE = "must meet custom requirements";
  ...
  public InvalidPolicy validate(@Nullable User user, String password) {
    ...
    if (!meetsMyRequirements(password)) {
      return new InvalidPolicy(ERROR_MESSAGE);
    }
    return null;
  }

  private boolean meetsMyRequirements(String password) {
    // your logic here
    return true;
  }
...
The `validate` method should return `null` if the password is valid, or an `InvalidPolicy` instance carrying the error
message otherwise.

If the policy should be configurable via `user.password.<id>.enabled`, override `getPolicyId()` and optionally
`isEnabledByDefault()`:
...
@Override
public String getPolicyId() {
  return "myPolicy"; // maps to user.password.myPolicy.enabled
}

@Override
public boolean isEnabledByDefault() {
  return false; // opt-in policy
}
----

Then, in your application's Guice module, add the policy :
...
public class MyModule extends AxelorModule {
  ...
  protected void configure() {
    addPasswordPolicy(MyCustomPasswordPolicy.class);
    //...
  }
}
----

The policy will be picked up automatically by the password manager alongside the built-in policies.

== Password Reset

There is built-in support for password reset functionality. This allows users to request a password reset link if they
have forgotten their password. This feature can be configured using the following properties:

[cols="4,8,1"]
|===
| Property                             | Description                                                      | Default
| `application.reset-password.enabled` | Whether to enable the password reset feature from the login page | true
| `application.reset-password.max-age` | Maximum validity period of password reset tokens in minutes        | 30
|===

=== User Experience

When enabled, a "Forgot password?" link appears on the login page. Users can click this link to enter their email
address and request a password reset link. The link remains valid for the duration specified by
`application.reset-password.max-age`.

=== Security Considerations

In production mode, to prevent host injection attacks, the password reset feature requires a configured application
host. It will not use the request host to determine the application's base URL when creating password reset links.
This means:

- The `application.base-url` configuration is required in most cases.
- Exception: When multi-tenancy is enabled, the system uses the first host in the `hosts` configuration for each tenant
instead.
