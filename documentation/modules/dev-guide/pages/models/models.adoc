...
<domain-models xmlns="http://axelor.com/xml/ns/domain-models"
  ...
  https://axelor.com/xml/ns/domain-models/domain-models_8.2.xsd">
...
IMPORTANT: Java reserved keywords can't be used as field names. PostgreSQL reserved keywords can't be used as column
names.
...
* **Recommended:** Use `SINGLE` for simple hierarchies or `JOINED` for complex data models requiring strict data
integrity.
* **Caution with `CLASS`:** While the `CLASS` strategy may seem convenient, it has significant drawbacks related to
performance: queries targeting the parent entity can become extremely slow as data grows (due to `UNION ALL`
operations).
====
...
The `encrypted` field values are stored in the database using AES-256 with `PBKDF2WithHmacSHA256`
key derivation. See <<Field Encryption>> for configuration and column size constraints.
...
The `<uuid>` field is used to define fields to store universally unique identifiers using `java.util.UUID` java type.

example:

[source,xml]
----
<uuid name="productUid" />
----
...
Field encryption stores sensitive values in the database using AES-256. The encryption key is derived from a password
using `PBKDF2WithHmacSHA256` with 600,000 iterations.

The default operation mode is *AES/GCM* (authenticated encryption), which provides both confidentiality and integrity
— any tampering with the stored value is detected on decryption. The mode can be changed to `CBC` via
`encryption.algorithm`, but GCM is strongly recommended.

The ciphertext is self-describing: the iteration count, salt size, and IV size are embedded in the payload. This means
that changing the `encryption.password` or `encryption.algorithm` settings in the future does not break existing stored
values, as long as the old password is provided via `encryption.old-password` during migration.
...
# change this password to a unique, long, and complex string
encryption.password = MySuperSecretKey

# Set encryption algorithm: CBC or GCM (default)
#encryption.algorithm = GCM
----

Mark `<string>` and `<binary>` fields as encrypted in the model definition:
...
Encrypted values are Base64-encoded in the database and are longer than the original plaintext. The table below gives
the maximum plaintext size that fits in a standard `VARCHAR(255)` column:

[cols="1,1,1", options="header"]
|===
| Mode | Max plaintext bytes | Notes

| GCM (default) | 132 B | Recommended — authenticated encryption
| CBC | 143 B | Legacy mode, no integrity check
|===

NOTE: For single-byte encodings (ASCII, digits, symbols), bytes equal characters. Multi-byte UTF-8 characters reduce
the limit proportionally: 2-byte characters (e.g. `é`) halve it to ~66 chars, 3-byte characters (e.g. `中`) reduce it
to ~44 chars.

If 132 bytes is insufficient for a given field, increase the column size in the database schema (e.g. `VARCHAR(512)`)
rather than switching to CBC mode.
