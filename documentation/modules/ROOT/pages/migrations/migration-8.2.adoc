...
* Non-admin users can only see and manage their own records. Listings, form views, and related-field lookups no longer
expose records belonging to other users.
* Creation, update, duplication, export, and mass-update through the generic data layer are disabled for everyone,
including administrators. These records are managed exclusively through the dedicated MFA and user-token screens and
services.
...
NOTE: Callers that only use `Selector.fetch(int, int)` are unaffected — the output map shape is unchanged, with the
