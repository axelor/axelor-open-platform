...
function ManyToOneCtrl($scope, $element, DataSource, ViewService) {
  ...
  $scope.findRelativeFields = (function () {
    ...
    return function () {
      ...
      relatives = $element.parents().find('[x-field][x-path^="'+path+'."]:not(label,.panel,.btn,.spacer-item,.separator-item)').map(function() {
    ...
    scope.formatItem = function(item) {
      if (item) {
        ...
        return typeof value === "string"
          ? axelor.sanitize(value).replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&')
          : value;
      }
      return "";
    };
