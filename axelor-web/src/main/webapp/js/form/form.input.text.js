...
ui.formInput('JsonField', 'String', {
	...
	link: function (scope, element, attrs, model) {
		...
		function onRender() {
			...
			scope[jsonName] = value ? angular.fromJson(value) : getDefaultValues();
		}

		model.$render = onRender;
		scope.$on('on:new', onRender);
		scope.$on('on:edit', function () {
			if (scope.viewType === 'form') onRender();
		});

		scope.$watch(jsonName, function (current, old) {
			var a = current || null;
			var b = old || null;
			if (a && a !== b && !angular.equals(a, defaultValues)) {
				scope.setValue(current ? angular.toJson(current) : current, true);
			}
		}, true);
	}
});
