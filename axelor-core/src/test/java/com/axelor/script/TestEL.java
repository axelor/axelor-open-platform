...
public class TestEL extends ScriptTest {
	...
	private static final String EXPR_INTERPOLATION = "fmt:text('(%s) = %s %s (%s) = (%s) ', title.name, firstName, lastName, fullName, __user__)";

	private static final String EXPR_CONCAT = "'(' += title.name += ') = ' += firstName += ' ' += lastName += ' (' += fullName += ') = (' += str(__user__) += ')' ";

	// false all, to evaluate all conditions
	private static final String EXPR_CONDITION = "(is(title, Contact) || fullName == 'foo') || is(__ref__, Title) || (__parent__ == null) || (__self__ == __this__)";
	...
	public void test14_java_concat() {
		...
		for (int i = 0; i < COUNT; i++) {
			String result = "(" + contact.getTitle().getName() + ") =" + " "
