...
import com.axelor.ical.db.ICalendarUser;
import com.axelor.ical.db.repo.ICalendarEventRepository;
import com.axelor.ical.db.repo.ICalendarUserRepository;
...
public class ICalendarService {
	...
	private String getValue(Component component, String name) {
	...
	private ICalendarEvent findOrCreateEvent(VEvent vEvent) {
		...
		event.setStartDate(new LocalDateTime(dtStart.getDate()));
		event.setEndDate(new LocalDateTime(dtEnd.getDate()));
		event.setAllDay(!(dtStart.getDate() instanceof DateTime));

		event.setSummary(getValue(vEvent, Property.SUMMARY));
	...
	private ICalendarUser findOrCreateUser(Property source) {
		URI addr = null;
	...
	private <T extends Property> T updateUser(T target, ICalendarUser user) {
	...
	private String mailto(final String mail, boolean strip) {
		if (mail == null) {
			return null;
		}
	...
	private Date toDate(LocalDateTime dt, boolean allDay) {
	...
	private VEvent createVEvent(ICalendarEvent event) throws SocketException {
		boolean allDay = event.getAllDay() == Boolean.TRUE;

		Date start = toDate(event.getStartDate(), allDay);
		Date end = toDate(event.getEndDate(), allDay);
		...
		if (event.getSummary() != null) {
			items.add(new Summary(event.getSummary()));
		}
	 ...
	 */
	public void export(ICalendar calendar, File file) throws IOException, ValidationException {
		Preconditions.checkNotNull(calendar, "calendar can't be null");
		Preconditions.checkNotNull(file, "input file can't be null");
	 ...
	 * @throws ValidationException
	 */
	public void export(ICalendar calendar, Writer writer) throws IOException, ValidationException {
		Preconditions.checkNotNull(calendar, "calendar can't be null");
	...
	private ICalendar doSync(ICalendar calendar, CalDavCalendarCollection collection)
		...
		for (ICalendarEvent item : calendar.getEvents()) {
			...
			VEvent target = remoteEvents.get(source.getUid());
			if (target == null) {
				target = source;
			}
			if (keepRemote) {
