...
public class ImplicitNamingStrategyImpl extends ImplicitNamingStrategyLegacyHbmImpl {
	...
	private Identifier determineContraintName(ImplicitConstraintNameSource source, String prefixOrSuffix) {
		...
		final List<Identifier> columns = source.getColumnNames()
				.stream()
		...
		if (prefixOrSuffix.startsWith("_")) {
			final String joined = columns.stream().map(Identifier::getText).collect(Collectors.joining("_"));
			return toIdentifier(tableName + "_" + joined + prefixOrSuffix, source.getBuildingContext());
		}

		return toIdentifier(NamingHelper.INSTANCE.generateHashedConstraintName(prefixOrSuffix, tableName, columns),
